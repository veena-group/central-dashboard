...
export class MyDocumentsComponent {
  ...
  constructor() {
    this.load(0);
    this.api.listCategories('DOCUMENT').subscribe((categories) => this.categories.set(categories));
  ...
  load(page: number): void {
    ...
    const year = this.yearControl.value.trim();
    this.api.listPaged<DocumentRow>('/documents', page, MyDocumentsComponent.PAGE_SIZE, {
  ...
  hasActiveFilters(): boolean {
    return !!this.searchControl.value.trim() || !!this.yearControl.value.trim() || !!this.categoryFilter() || !!this.visibilityFilter();
  }
  ...
  openAttachmentList(row: DocumentRow): void {
